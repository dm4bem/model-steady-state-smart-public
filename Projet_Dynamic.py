...
Ta,Tb,Tc = 20,20,20
b[controller] = Ta, Tb, Tc  # °C setpoint temperature of the rooms
...
θ_1 = [f'θ{i}' for i in range(nq)]
q_1 = [f'q{j}' for j in range(nθ)]

A_pd = pd.DataFrame(A, index=θ_1, columns=q_1)
G_pd = pd.Series(G, index=θ_1)
C_pd = pd.Series(C, index=q_1)
b_pd = pd.Series(b, index=θ_1)
f_pd = pd.Series(f, index=q_1)
y_pd = pd.Series(y, index=q_1)
...
T_ext0 = To * np.ones(n)        # outdoor temperature
T_ext1 = To * np.ones(n)        # outdoor temperature
T_ext2 = To * np.ones(n)        # outdoor temperature
T_ext3 = To * np.ones(n)        # outdoor temperature
T_ext4 = To * np.ones(n)        # outdoor temperature
T_ext5 = To * np.ones(n)        # outdoor temperature
T_ext6 = To * np.ones(n)        # outdoor temperature
T_ext7 = To * np.ones(n)        # outdoor temperature
T_ext8 = To * np.ones(n)        # outdoor temperature

T_controle0 = 20 * np.ones(n)     # indoor temperature set point A
T_controle1 = 20 * np.ones(n)     # indoor temperature set point B
T_controle2 = 20 * np.ones(n)     # indoor temperature set point C

Φa0 = α_gSW*S_fenetre*E * np.ones(n)         # solar radiation absorbed by the glass A
Φa1 = α_gSW*S_fenetre*E * np.ones(n)         # solar radiation absorbed by the glass B
Φa2 = α_gSW*2*S_fenetre*E * np.ones(n)         # solar radiation absorbed by the glass C

Φo0 = α_wSW*S_A_mur_ext*E * np.ones(n)     # solar radiation absorbed by the outdoor surface of the wall A
Φo1 = α_wSW*S_B_mur_ext*E * np.ones(n)     # solar radiation absorbed by the outdoor surface of the wall B
Φo2 = α_wSW*S_B_mur_ext*E * np.ones(n)   # solar radiation absorbed by the outdoor surface of the wall C

Φi = np.zeros(n)            # solar radiation absorbed by the indoor surface of the wall
Qa = np.zeros(n)        # auxiliary heat gains

data = {'T_ext0': T_ext0, 'T_ext1': T_ext1, 'T_ext2': T_ext2, 'T_ext3': T_ext3,
        'T_ext4': T_ext4, 'T_ext5': T_ext5, 'T_ext6': T_ext6, 'T_ext7': T_ext7, 
        'T_ext8': T_ext8, 'T_controle0': T_controle0, 'T_controle1': T_controle1, 
        'T_controle2': T_controle2,'Φo0': Φo0, 'Φo1': Φo1, 'Φo2': Φo2, 'Φo0': Φo0, 'Φi': Φi, 
        'Qa': Qa, 'Φa0': Φa0, 'Φa1': Φa1, 'Φa2': Φa2}

input_data_set = pd.DataFrame(data, index=time)

print('us :',us)

print('us.index :',us.index)

print('data :',data)

print('input_data :',input_data_set)

print('input_data.index :',input_data_set.index)
